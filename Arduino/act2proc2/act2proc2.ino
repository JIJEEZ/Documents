...
void loop() {
  Serial.print("Enter an integer, float, character or byte: ");
  ...
  if (userInput.length() == 1 && !isDigit(userInput.charAt(0))) {
    ...
    String binaryToChar = applyBinaryConversion(charInput);
    Serial.print("Binary representation of char '");
    Serial.print(charInput);
    Serial.print("': ");
    Serial.println(binaryToChar);
    ...
    if (inputValue < 0 && binaryOutput.length() < 32) {
      binaryOutput = applyTwosComplement(binaryOutput, decplaces);
    }
...
String applyTwosComplement(String binary, int minDecimalPlaces) {
  ...
  while (fracPart.length() < minDecimalPlaces) {
    fracPart += '0';
  }
